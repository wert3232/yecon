/**
 * @Title: RaiseHondaCamryParse.java
...
/**
 * @ClassName: RaiseHondaCamryParse
 * @Description: TODO
 * @author hzGuo
 * @date 2016年4月18日 上午10:20:27
 *
 */
public class RaiseHondaCamryParse extends CANBusParse {

	public static final int RAISE_HONDA_CAMRY_DATACMD_AIR = 0x28;
	public static final int RAISE_HONDA_CAMRY_DATACMD_CAR = 0x24;
	public static final int RAISE_HONDA_CAMRY_DATACMD_SPEED = 0x21;
	...
	public boolean ParseData(int iDataCmd, int[] iDataBuffer) {
		...
		try {
			switch (iDataCmd) {
			case RAISE_HONDA_CAMRY_DATACMD_AIR:
				// data 0
				...
				getAir().setMaxFrontLight(GetBit(iDataBuffer[0], 2)); // 温度双驱控制
				...
				getAir().setRightTemp(iDataBuffer[3]); // 右温度值 (实际温度值 = 温度值
															...
															// 温度步进)
				// data 4
				getAir().setFrontWindowDemisting(GetBit(iDataBuffer[4], 7)); // 前窗除雾
				getAir().setRearWindowDemisting(GetBit(iDataBuffer[4], 6)); // 后窗加热
				getAir().setAQS(GetBit(iDataBuffer[4], 5));	// AQS
				getAir().setECO(GetBit(iDataBuffer[4], 4)); // ECO
				getAir().setACMax(GetBit(iDataBuffer[4], 3)); // AC MAX
				getAir().setTempUnit(GetBit(iDataBuffer[4], 0)); // 温度单位
				break;
			case RAISE_HONDA_CAMRY_DATACMD_CAR:
				// data 0
				...
				break;
			case RAISE_HONDA_CAMRY_DATACMD_SPEED:
				getCar().setDrivenDistance((iDataBuffer[4] << 8 | iDataBuffer[5]));
				break;
			default:
				bRet = false;
				break;
	...
	public void LaunchCanbusUI(Context content, int iDataCmd) {
		// TODO Auto-generated method stub
		if (iDataCmd == RAISE_HONDA_CAMRY_DATACMD_AIR) {
			if (!isForeground(content, HondaCamryACActivity.class.getName())) {
				Intent intent = new Intent(content, HondaCamryACActivity.class);
