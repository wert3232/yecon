...
public class ReCQParser extends CanProxy{
	private Handler    	  mObjHandler;
	private ReCQProtol	  mObjProtocol;
	...
	public void start(Handler handler, Context context, String name,
		...
		this.mObjProtocol = (ReCQProtol)protocol;
	}
	
	@Override
	public void Init(E_CMD_TYPE eCmdType) {
		if (eCmdType == E_CMD_TYPE.eCmd_Type_PopWind || eCmdType == E_CMD_TYPE.eCmd_Type_BackCar) {
			super.RegisterProxy(ReCQProtol.DATA_TYPE_BACK_LIGHT_INFO, ReCQProtol.DATA_TYPE_BACK_LIGHT_INFO);
			super.RegisterProxy(ReCQProtol.DATA_TYPE_BASE_INFO, ReCQProtol.DATA_TYPE_BASE_INFO);
			super.RegisterProxy(ReCQProtol.DATA_TYPE_WHEEL_INFO, ReCQProtol.DATA_TYPE_WHEEL_INFO);
			super.RegisterProxy(ReCQProtol.DATA_TYPE_WHEEL_KEY_INFO, ReCQProtol.DATA_TYPE_WHEEL_KEY_INFO);
			super.RegisterProxy(ReCQProtol.DATA_TYPE_REAR_RADAR_INFO, ReCQProtol.DATA_TYPE_REAR_RADAR_INFO);
			super.RegisterProxy(ReCQProtol.DATA_TYPE_FRONT_RADAR_INFO, ReCQProtol.DATA_TYPE_FRONT_RADAR_INFO);
			super.RegisterProxy(ReCQProtol.DATA_TYPE_AIR_SET_INFO, ReCQProtol.DATA_TYPE_AIR_SET_INFO);
			super.RegisterProxy(ReCQProtol.DATA_TYPE_CAR_SET_INFO, ReCQProtol.DATA_TYPE_CAR_SET_INFO);
			super.RegisterProxy(ReCQProtol.DATA_TYPE_PANEL_INFO, ReCQProtol.DATA_TYPE_PANEL_INFO);
		}
	}
	...
	public void handleMessage(Message msg) {
		...
		case CanTxRxStub.MSG_CAN_RX:
			...
			if (iCmdID == mObjProtocol.GetAirInfoCmdId()) {
				...
				if (mDoorData != packet[3]) {
					...
					mObjHandler.sendMessage(objUImsg);
						...
						iCmdID == ReCQProtol.DATA_TYPE_PANEL_INFO) {
			...
			} else if (iCmdID == ReCQProtol.DATA_TYPE_CAR_SET_INFO) {
