...
public class AVInExtActivity extends Activity implements View.OnClickListener,
    ...
    enum state {
        create, resume, start, restart, pause, stop, destroy
    };
    ...
    private void SetAuxVideoPara() {
        /*
         * if(!this.isResumed()){ return; } hue_values = DEFAULT_AUX_VALUES[0]; saturation_values =
         * DEFAULT_AUX_VALUES[1]; contrast_values = DEFAULT_AUX_VALUES[2]; brightness_values =
         * DEFAULT_AUX_VALUES[3]; aux_exist = 1; SystemProperties.set(aux_tag, aux_exist+"");
         * hue_values = Integer.parseInt(SystemProperties.get(hue_tag, hue_values+""));
         * saturation_values = Integer.parseInt(SystemProperties.get(saturation_tag,
         * saturation_values+"")); contrast_values =
         * Integer.parseInt(SystemProperties.get(contrast_tag, contrast_values+"")) ;
         * brightness_values = Integer.parseInt(SystemProperties.get(auxbrightness_tag,
         * brightness_values+"")); AtcSettings.Display.SetHueLevel(hue_values);
    ...
    private void SetSysVideoPara() {
        /*
         * hue_values = DEFAULT_SYS_VALUES[0]; saturation_values = DEFAULT_SYS_VALUES[1];
         * contrast_values = DEFAULT_SYS_VALUES[2]; brightness_values = DEFAULT_SYS_VALUES[3];
         * aux_exist = 0; SystemProperties.set(aux_tag, aux_exist+""); hue_values =
         * Settings.System.getInt(getContentResolver(),Settings.System.SCREEN_HUE, hue_values);
         * saturation_values =
         * Settings.System.getInt(getContentResolver(),Settings.System.SCREEN_SATURATION,
         * saturation_values); contrast_values =
         * Settings.System.getInt(getContentResolver(),Settings.System.SCREEN_CONTRAST,
         * contrast_values); brightness_values =
         * Integer.parseInt(SystemProperties.get(brightness_tag, brightness_values+""));
            ...
            switch (msg) {
                case AVIN.SIGNAL_READY:
                    ...
                    if (mAvinsurfaceview4Front != null) {
                        Log.d(TAG, "............... param1 =" + param1 + "...............param2 +"
                                + param2);
                    ...
                    synchronized (mCbmLock) {
                        ...
                        if (destDir == mDestDir) {
                            ...
                            if (mCbmCtlStatus == InputSource.CBM_CONTROL_STOPED) {
                                L.i(TAG, "OnCbmCmdListener CBM_STOP Already Stoped mDestDir:"
                                        + mDestDir);
                                return;
                            }
                            if (mCurrentStatus == InputSource.STATUS_STOPED) {
                                L.i(TAG,
                                        "OnCbmCmdListener CBM_STOP Already been stoped because there is other activity in front");
                                return;
                            }
                            showSurfaceView(View.INVISIBLE);
    ...
    BroadcastReceiver mQuickBootListener = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            else if ("action_lightactivity.resume".equals(action))
            {
                if (avinV_signal_on && eActivityState == state.stop)
                {
                    myHandler.sendEmptyMessageDelayed(SET_AUX_VIDEO_PARA,
                            AVINConstant.SET_AUX_DELAY_TIME);
                }
            }
    ...
    public boolean onTouch(View arg0, MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (!avinV_signal_on || mIsFullScreen == true) {// if full screen then show top bar
                                                                // or display
                    showTopBar(0);
                } else {
                    hideTopBar(0);
