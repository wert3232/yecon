...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    protected void init() {
        ...
        L.d("vvvvvv", "mDensityDpi==="+mDensityDpi);
    ...
    public void setOnTouchListener(OnTouchListener l) {
        ...
        final int count = getPageCount();
        for (int i = 0; i < count; i++) {
            getPageAt(i).setOnTouchListener(l);
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (DEBUG) L.d(TAG, "PagedView.onMeasure(): " + widthSize + ", " + heightSize);
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (DEBUG) L.d(TAG, "\tmeasure-child" + i + ": " + child.getMeasuredWidth() + ", "
        ...
        if (childCount > 0) {
            if (DEBUG) L.d(TAG, "getRelativeChildOffset(): " + getMeasuredWidth() + ", "
                    + getChildWidth(0));
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (DEBUG) L.d(TAG, "PagedView.onLayout()");
        final int verticalPadding = getPaddingTop() + getPaddingBottom();
        final int childCount = getChildCount();
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child.getVisibility() != View.GONE) {
                ...
                if (DEBUG) L.d(TAG, "\tlayout-child" + i + ": " + childLeft + ", " + childTop);
            ...
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if (Math.abs(deltaX) >= 1.0f) {
                    ...
                    if (!mDeferScrollUpdate) {
                        ...
                        if (DEBUG) L.d(TAG, "onTouchEvent().Scrolling: " + deltaX);
                    } else {
                        invalidate();
                    }
                    mLastMotionX = x;
    ...
    protected void snapToPageWithVelocity(int whichPage, int velocity) {
        ...
        if (DEBUG) L.d(TAG, "snapToPage.getChildOffset(): " + getChildOffset(whichPage));
        if (DEBUG) L.d(TAG, "snapToPageWithVelocity.getRelativeChildOffset(): "
    ...
    protected void snapToPage(int whichPage, int duration) {
        ...
        if (DEBUG) L.d(TAG, "snapToPage.getChildOffset(): " + getChildOffset(whichPage));
        if (DEBUG) L.d(TAG, "snapToPage.getRelativeChildOffset(): " + getMeasuredWidth() + ", "
    ...
    protected void loadAssociatedPages(int page, boolean immediateAndOnly) {
        if (mContentIsRefreshable) {
            ...
            if (page < count) {
                ...
                if (DEBUG) L.d(TAG, "loadAssociatedPages: " + lowerPageBound + "/"
                        + upperPageBound);
