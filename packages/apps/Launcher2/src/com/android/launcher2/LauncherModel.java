...
public class LauncherModel extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (DEBUG_LOADERS) L.d(TAG, "onReceive intent=" + intent);
        ...
        if (Intent.ACTION_PACKAGE_CHANGED.equals(action)
            ...
            forceReload();
             ...
             if (mPreviousConfigMcc != currentConfig.mcc) {
                   L.d(TAG, "Reload apps on config change. curr_mcc:"
    ...
    public void startLoader(boolean isLaunching, int synchronousBindPage) {
        synchronized (mLock) {
            if (DEBUG_LOADERS) {
                L.d(TAG, "startLoader isLaunching=" + isLaunching);
            }
    ...
    private class LoaderTask implements Runnable {
        ...
        private void loadAndBindWorkspace() {
            ...
            // Load the workspace
            if (DEBUG_LOADERS) {
                L.d(TAG, "loadAndBindWorkspace mWorkspaceLoaded=" + mWorkspaceLoaded);
            }
                ...
                mHandler.postIdle(new Runnable() {
                        public void run() {
                            synchronized (LoaderTask.this) {
                                ...
                                if (DEBUG_LOADERS) {
                                    L.d(TAG, "done with previous binding step");
                                }
        ...
        public void run() {
            ...
            keep_running: {
                ...
                synchronized (mLock) {
                    if (DEBUG_LOADERS) L.d(TAG, "Setting thread priority to " +
                            (mIsLaunching ? "DEFAULT" : "BACKGROUND"));
                    android.os.Process.setThreadPriority(mIsLaunching
                            ? Process.THREAD_PRIORITY_DEFAULT : Process.THREAD_PRIORITY_BACKGROUND);
                }
              //  if (loadWorkspaceFirst) {
              //      if (DEBUG_LOADERS) L.d(TAG, "step 1: loading workspace");
                    loadAndBindWorkspace();
              //  } else {
              //      if (DEBUG_LOADERS) L.d(TAG, "step 1: special: loading all apps");
              //      loadAndBindAllApps();
              //  }
                ...
                synchronized (mLock) {
                    if (mIsLaunching) {
                        if (DEBUG_LOADERS) L.d(TAG, "Setting thread priority to BACKGROUND");
                ...
                // second step
                //if (loadWorkspaceFirst) {
                //    if (DEBUG_LOADERS) L.d(TAG, "step 2: loading all apps");
                    loadAndBindAllApps();
                //} else {
                //    if (DEBUG_LOADERS) L.d(TAG, "step 2: special: loading workspace");
                //    loadAndBindWorkspace();
                //}
            ...
            if (DEBUG_LOADERS) L.d(TAG, "Comparing loaded icons to database icons");
            synchronized (sBgLock) {
        ...
        private boolean checkItemPlacement(ItemInfo occupied[][][], ItemInfo item) {
            ...
            if (item.container == LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
                ...
                if (occupied[Launcher.SCREEN_COUNT][item.screen][0] != null) {
                    L.e(TAG, "Error loading shortcut into hotseat " + item
            ...
            for (int x = item.cellX; x < (item.cellX+item.spanX); x++) {
                for (int y = item.cellY; y < (item.cellY+item.spanY); y++) {
                    if (occupied[containerIndex][x][y] != null) {
                        L.e(TAG, "Error loading shortcut " + item
                        ...
                        try {
                            ...
                            case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
                                ...
                                if (info != null) {
                                    ...
                                    L.e(TAG, "Error loading shortcut " + id + ", removing it");
                            ...
                            case LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET:
                                ...
                                if (!isSafeMode && (provider == null || provider.provider == null ||
                                    ...
                                    if (container != LauncherSettings.Favorites.CONTAINER_DESKTOP &&
                                        container != LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
                                        L.e(TAG, "Widget found where container != " +
                            ...
                            L.w(TAG, "Desktop items loading interrupted:", e);
                        }
                    }
                } finally {
                    c.close();
                ...
                if (itemsToRemove.size() > 0) {
                    ...
                    for (long id : itemsToRemove) {
                        ...
                        try {
                            ...
                            L.w(TAG, "Could not remove id = " + id);
                        }
                    }
                }

                if (DEBUG_LOADERS) {
                    L.d(TAG, "loaded workspace in " + (SystemClock.uptimeMillis()-t) + "ms");
                    L.d(TAG, "workspace layout: ");
        ...
        private void bindWorkspace(int synchronizeBindPage) {
            ...
            if (oldCallbacks == null) {
                ...
                L.w(TAG, "LoaderTask running with no launcher");
                return;
            }
        ...
        private void loadAndBindAllApps() {
            if (DEBUG_LOADERS) {
                L.d(TAG, "loadAndBindAllApps mAllAppsLoaded=" + mAllAppsLoaded);
            }
            if (!mAllAppsLoaded) {
                loadAllAppsByBatch();
                synchronized (LoaderTask.this) {
        ...
        private void onlyBindAllApps() {
            ...
            if (oldCallbacks == null) {
                // This launcher has exited and nobody bothered to tell us.  Just bail.
                L.w(TAG, "LoaderTask running with no launcher (onlyBindAllApps)");
                return;
            }
            ...
            Runnable r = new Runnable() {
                public void run() {
                    ...
                    if (DEBUG_LOADERS) {
                        L.d(TAG, "bound all " + list.size() + " apps from cache in "
                                + (SystemClock.uptimeMillis()-t) + "ms");
                    }
                }
            };
			...
			while (i < N && !mStopped) {
				...
				for (int j = 0; i < N && j < batchSize;) {
					...
					if (!mAlreadyInDesktopApp.isExistedShortcutFlag(
                            ...
                            appInfo.componentName.getClassName()) && !isMaskClassName(appInfo)) {
				...
				mHandler.post(new Runnable() {
					public void run() {
						if (callbacks != null && shortcuts.size() > 0) {
							...
							L.i(TAG, "not binding apps: no Launcher activity");
						}
					}
				});
        ...
        private void loadAllAppsByBatch() {
            ...
            if (oldCallbacks == null) {
                // This launcher has exited and nobody bothered to tell us.  Just bail.
                L.w(TAG, "LoaderTask running with no launcher (loadAllAppsByBatch)");
                return;
            }
            ...
            while (i < N && !mStopped) {
                if (i == 0) {
                    ...
                    apps = packageManager.queryIntentActivities(mainIntent, 0);
                    if (DEBUG_LOADERS) {
                        L.d(TAG, "queryIntentActivities took "
                    ...
                    N = apps.size();
                    if (DEBUG_LOADERS) {
                        L.d(TAG, "queryIntentActivities got " + N + " apps");
                    }
                    if (N == 0) {
                        // There are no apps?!?
                        return;
                ...
                mHandler.post(new Runnable() {
                    public void run() {
                        ...
                        if (callbacks != null) {
                            ...
                            if (DEBUG_LOADERS) {
                                L.d(TAG, "bound " + added.size() + " apps in "
                                    + (SystemClock.uptimeMillis() - t) + "ms");
                            }
                        } else {
                            L.i(TAG, "not binding apps: no Launcher activity");
                ...
                if (DEBUG_LOADERS) {
                    L.d(TAG, "batch of " + (i-startIndex) + " icons processed in "
                ...
                if (mAllAppsLoadDelay > 0 && i < N) {
                    try {
                        if (DEBUG_LOADERS) {
                            L.d(TAG, "sleeping for " + mAllAppsLoadDelay + "ms");
                        }
        ...
        public void dumpState() {
            synchronized (sBgLock) {
                L.d(TAG, "mLoaderTask.mContext=" + mContext);
                L.d(TAG, "mLoaderTask.mIsLaunching=" + mIsLaunching);
                L.d(TAG, "mLoaderTask.mStopped=" + mStopped);
                L.d(TAG, "mLoaderTask.mLoadAndBindStepFinished=" + mLoadAndBindStepFinished);
                L.d(TAG, "mItems size=" + sBgWorkspaceItems.size());
            }
        }
    }
    ...
    private class PackageUpdatedTask implements Runnable {
        ...
        public void run() {
            ...
            switch (mOp) {
                case OP_ADD:
                    for (int i=0; i<N; i++) {
                        if (DEBUG_LOADERS) L.d(TAG, "mAllAppsList.addPackage " + packages[i]);
                        mBgAllAppsList.addPackage(context, packages[i]);
                    }
                    break;
                case OP_UPDATE:
                    for (int i=0; i<N; i++) {
                        if (DEBUG_LOADERS) L.d(TAG, "mAllAppsList.updatePackage " + packages[i]);
                ...
                case OP_UNAVAILABLE:
                    for (int i=0; i<N; i++) {
                        if (DEBUG_LOADERS) L.d(TAG, "mAllAppsList.removePackage " + packages[i]);
            ...
            if (callbacks == null) {
                L.w(TAG, "Nobody to tell about the new app.  Launcher is probably loading.");
                return;
            }
                    ...
                    Runnable r = new Runnable() {
                        public void run() {
                            ...
                            if (callbacks == cb && cb != null
                                ...
                                L.i(TAG, "not Install apps to desktop");
                            }
                        }
                    };
                    runOnMainThread(r);
    ...
    public ShortcutInfo getShortcutInfo(PackageManager manager, Intent intent, Context context,
        ...
        try {
            ...
            L.d(TAG, "getPackInfo failed for package " + componentName.getPackageName());
        }
        ...
        if (bitmap != null && bitmap instanceof Bitmap) {
            ...
            if (extra != null && extra instanceof ShortcutIconResource) {
                try {
                    ...
                    L.w(TAG, "Could not load shortcut icon: " + extra);
                }
            }
        }
    ...
    void updateSavedIcon(Context context, ShortcutInfo info, byte[] data) {
        ...
        if (needSave) {
            L.d(TAG, "going to save icon bitmap for info=" + info);
  ...
  //by lzy
	final static String WHITE_LIST_PERSYS_FUN[] = {
		"persist.sys.dvd_eject_enable",
		"persist.sys.fun.fm.enable",
		"persist.sys.fun.gps.enable",
		"persist.sys.fun.avin.enable",
		"persist.sys.fun.avin2.enable",
		"persist.sys.fun.tv.enable",
		"persist.sys.fun.ipod.enable",
		"persist.sys.fun.music.enable",
		"persist.sys.fun.video.enable",
		"persist.sys.fun.bt.enable",
		"persist.sys.fun.dvr.enable",
		"persist.sys.fun.miracast.enable",
		"persist.sys.fun.canbus",
		"persist.sys.fun.tpms.enable",
	};
			
    
    static String WHITE_LIST_CLASSNAME[] = {
    	DVD_START_ACTIVITY,
	...
	private boolean isMaskClassName(ShortcutInfo appInfo)
	{
		int i = 0;
		for(String str : WHITE_LIST_CLASSNAME)
		{
			if(appInfo.componentName.getClassName().equals(WHITE_LIST_CLASSNAME[i])
					&& SystemProperties.getInt(WHITE_LIST_PERSYS_FUN[i], 0) == 0)
			    return true;
			i++;
		}
		if (appInfo.componentName.getPackageName().equalsIgnoreCase("com.phonelink.main")||
			appInfo.componentName.getPackageName().equalsIgnoreCase("com.phonelink.iplay")||
			appInfo.componentName.getPackageName().equalsIgnoreCase("com.baidu.che.codriver")
			)
			return true;
		return false;	
	}
	
	private boolean isMaskClassName(ApplicationInfo appInfo)
	{
		int i = 0;
		for(String str : WHITE_LIST_CLASSNAME)
		{
