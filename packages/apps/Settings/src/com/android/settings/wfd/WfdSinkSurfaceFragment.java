...
public final class WfdSinkSurfaceFragment extends DialogFragment implements
    ...
    public WfdSinkSurfaceFragment(WfdSettingsExt ext) {
        mExt = ext;
    }

    @Override
    public void onSaveInstanceState(Bundle outState) {
        Log.d(TAG, "[lingling]onSaveInstanceState");
        disconnect();
    ...
    public Surface getSinkSurface() {
       Log.d(TAG, "[lingling] mView is null? " + (mSinkView == null));
        return mSinkView != null ? mSinkView.getHolder().getSurface() : null;
    }

    @Override
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        Log.d(TAG, "[lingling]mDialog is null? " + (mDialog == null));

        mLatinTest = SystemProperties.get("wfd.uibc.latintest", "0").equals("1");
    ...
    private class FullScreenDialog extends Dialog {
        ...
        protected void onStart() {
            Log.d(TAG, "[lingling]dialog onStart");
            super.onStart();
            mSystemUiBak = mSinkViewLayout.getSystemUiVisibility();
            mSinkViewLayout
                    .setSystemUiVisibility(View.SYSTEM_UI_FLAG_HIDE_NAVIGATION);
			//WindowManager.LayoutParams wl = mActivity.getWindow().getAttributes();//00 add
            mActivity.getWindow().addFlags(
                    WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
			//wl.alpha = 0.6f;
			//mActivity.getWindow().setAttributes(wl);//00 add
            mSinkView.getHolder().addCallback(WfdSinkSurfaceFragment.this);
            mSinkViewLayout.setOnLongClickListener(WfdSinkSurfaceFragment.this);
        ...
        protected void onStop() {
            Log.d(TAG, "[lingling]dialog onStop");
        ...
        @Override
        public void dismiss() {
            Log.d(TAG, "[lingling]dialog dismiss");
            disconnect();
            super.dismiss();
        }
        ...
        public void onBackPressed() {
            Log.d(TAG, "[lingling]dialog onBackPressed");
            if (mGuideShowing) {
                removeWfdSinkGuide();
                return;
            }
    ...
    private class WfdSinkLayout extends FrameLayout {
        ...
        public boolean onGenericMotionEvent(MotionEvent event) {
            ...
            Log.d(TAG, "onGenericMotionEvent event.getSource()="
                    + event.getSource());
            //if (FeatureOption.MTK_WFD_SINK_UIBC_SUPPORT) {
        ...
        public boolean onKeyPreIme(int keyCode, KeyEvent event) {
           ...
           // if (FeatureOption.MTK_WFD_SINK_UIBC_SUPPORT) {
                ...
                if (asciiCode == 0 || asciiCode < 0x20) {
                    Log.d(TAG, "Can't find unicode for keyCode=" + keyCode);
                ...
                if (mLatinTest && keyCode == KeyEvent.KEYCODE_F1) {
                    Log.d(TAG, "Latin Test Mode enabled");
                ...
                Log.d(TAG, "onKeyPreIme asciiCode=" + asciiCode);
                if (asciiCode == 0x00) {
                    Log.d(TAG, "Can't find control for keyCode=" + keyCode);
                } else {
                    StringBuilder eventDesc = new StringBuilder();
                    eventDesc.append(
                            String.valueOf(GENERIC_INPUT_TYPE_ID_KEY_DOWN))
                            .append(",").append(
        ...
        private void sendUibcInputEvent(String eventDesc) {
            Log.d(TAG, "sendUibcInputEvent: " + eventDesc);
        ...
        private void removePendingCallback() {
           Log.d(TAG, "removePendingCallback");
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        Log.d(TAG, "[lingling]surface created");
        if (!mSurfaceShowing) {
    ...
    public void surfaceDestroyed(SurfaceHolder holder) {
        Log.d(TAG, "[lingling]surface destroyed");
        disconnect();
    }

    @Override
    public boolean onLongClick(View v) {
       Log.d(TAG, "[lingling]onLongClick");
        dismiss();
        return true;
    }
