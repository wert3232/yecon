...
extern int NativeMP3Decoder_init(char * filepath,int start);
 
 
jint Java_com_example_libmad_NativeMP3Decoder_init(JNIEnv *env, jobject obj, jstring file,jint startAddr)
{  
 ...
 jint Java_com_example_libmad_NativeMP3Decoder_getAudioBuf(JNIEnv *env, jobject obj ,jshortArray audioBuf,jint len)
{  
   int bufsize = 0;  
   int ret = 0;  
    if (audioBuf != NULL) {  
 ...
 jint Java_com_example_libmad_NativeMP3Decoder_getAudioSamplerate()
{  
   return NativeMP3Decoder_getAduioSamplerate();  
}  
 
 
void Java_com_example_libmad_NativeMP3Decoder_closeAduioFile( )
