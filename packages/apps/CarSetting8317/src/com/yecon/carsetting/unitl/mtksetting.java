...
public class mtksetting extends Service {
	...
	public static mtksetting getInstance() {
	...
	public static int getVolume(int Index) {
		...
		float decibels = curve[segment][1]
				...
				* ((curve[segment + 1][1] - curve[segment][1]) / (curve[segment + 1][0] - curve[segment][0]));

		int ret = (int) (Math.exp(decibels * 0.115129f) * 0x20000);
		// Log.d(TAG, "mtksetting getVolume multi "+(Math.exp( decibels *
		// 0.115129f)));
	...
	/*
	 * void syncBrightnessToHW() { SharedPreferences setting =
	 * getApplicationContext().getSharedPreferences(Array.MTKSettings,
	 * Context.MODE_PRIVATE); if( setting == null ) { Log.e(TAG,
	 * "mtk kkk syncBrightnessToHW setting is NULL"); return; } int
	 * backlightValue=setting.getInt("backlight", 55); //backlight value saved:
	 * 0 ~ 100 Log.d(TAG,
	 * "mtk kkk3 syncBrightnessToHW backlightValue="+backlightValue);
	 * Settings.System.putInt(getApplicationContext().getContentResolver(),
	 * Settings.System.SCREEN_BRIGHTNESS, backlightValue*235/100 + 20);
	 * if(backlightValue < 5){ AtcSettings.Display.SetBackLightLevel(20); }else{
	 * AtcSettings.Display.SetBackLightLevel(backlightValue); //backlight value
	 * set to HW: 5~100 } }
	 */// 00
	...
	/** Display setting */
	/*
	 * public int GSetBrightnessLevel(int level) { Log.d(TAG,
	 * "GSetBrightnessLevel"); return
	 * AtcSettings.Display.SetBrightnessLevel(level); }
	 */
	...
	/*
	 * public int GSetBackLightLevel(int level) { Log.d(TAG,
	 * "GSetBackLightLevel"); return
	 * AtcSettings.Display.SetBackLightLevel(level); }
	 */
	...
	public int SetBalance(int u4Values, int eBalanceType) {
		...
		return AtcSettings.Audio.SetBalance(
				AtcSettings.Audio.BalanceType.nativeToType(eBalanceType), u4Values);
	}

	public int SetReverbType(int eReverbType, int[] ReverbCoef) {
		Log.d(TAG, "SetReverbType");
		return AtcSettings.Audio.SetReverb(AtcSettings.Audio.ReverbType.nativeToType(eReverbType),
				ReverbCoef);
	}

	public int SetTestTone(int eTestTone, int eTestToneType) {
		Log.d(TAG, "SetTestTone");
		return AtcSettings.Audio.SetTestTone(
				AtcSettings.Audio.TestToneMainType.nativeToType(eTestTone),
	...
	public int SetLoudNess(int uLoudNessType, int[] aiLoudNessGains) {
		...
		return AtcSettings.Audio.SetLoudness(
				AtcSettings.Audio.LoudnessMode.nativeToType(uLoudNessType), aiLoudNessGains);
	}
	...
	public int SetAudFeature(int eAudFeatur) {
		...
		return AtcSettings.Audio.SetDecFeatureType(AtcSettings.Audio.DecFeatureType
				.nativeToType(eAudFeatur));
	}
	...
	public int SetSRSSwitch(int eCSIISwitch) {
		...
		return AtcSettings.Audio.SetSRSSwitchType(AtcSettings.Audio.SRSSwitchType
				.nativeToType(eCSIISwitch));
	}
	...
	public int SetSRSFocus(int eFocus, int eCSIISwitch) {
		...
		return AtcSettings.Audio.SetSRSFocusOn(AtcSettings.Audio.SRSFocusType.nativeToType(eFocus),
				isOn);
	}
	...
	public int SetSRSTrueBassSize(int eTBSS, int eCSIITBSS) {
		...
		return AtcSettings.Audio.SetSRSTrueBassSize(
				AtcSettings.Audio.SRSTrueBassSizeType.nativeToType(eTBSS), eCSIITBSS);
	}
	...
	public int ChooseSpdifOutput(int eOutType) {
		...
		return AtcSettings.Audio.ChooseSpdifOutput(AtcSettings.Audio.SpdifOutputType
				.nativeToType(eOutType));
	}
