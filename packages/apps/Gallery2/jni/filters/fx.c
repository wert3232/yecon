...
#define  ALOGI(...)  __android_log_print(ANDROID_LOG_INFO,"GalleryJNI->fx.c",__VA_ARGS__)

__inline__ int  interp(unsigned char  *src, int buf_sz, int p , int *off ,float dr,float dg, float db){
    ...
    for (i = 0; i < 8; i++)
    {
        if ((p + off[i]) >= buf_sz)
        {
            ALOGI("interp -> Oh, My God! offset >buf_sz\n");
            new_off[i] = buf_sz - 1;
        }
        else if ((p + off[i]) < 0)
        {
            ALOGI("interp -> Oh, My God! offset < 0\n");
            new_off[i] = 0;
        }
        else
        {
    ...
    float fr00 = (src[new_off[0]])*(1-dr)+(src[new_off[1]])*dr;
    //float fr01 = (src[p+off[2]])*(1-dr)+(src[p+off[3]])*dr;
    float fr01 = (src[new_off[2]])*(1-dr)+(src[new_off[3]])*dr;
    //float fr10 = (src[p+off[4]])*(1-dr)+(src[p+off[5]])*dr;
    float fr10 = (src[new_off[4]])*(1-dr)+(src[new_off[5]])*dr;
    //float fr11 = (src[p+off[6]])*(1-dr)+(src[p+off[7]])*dr;
    float fr11 = (src[new_off[6]])*(1-dr)+(src[new_off[7]])*dr;
...
void JNIFUNCF(ImageFilterFx, nativeApplyFilter, jobject bitmap, jint width, jint height, jobject lutbitmap,jint lutwidth, jint lutheight )
{
    ...
    int lutbuf_sz = lutwidth * lutheight * STEP;
    ALOGI("ImageFilterFx->nativeApplyFilter: width = %d, height = %d, lutwidth = %d, lutheight = %d, lutbuf_sz = %d\n", width, height, lutwidth, lutheight, lutbuf_sz);
    ...
    for (i = 0; i < len; i+=STEP)
    {
        ...
        float dg = fg-lut_g;
        float db = fb-lut_b;
        rgb[RED]   = clamp(interp(lutrgb, lutbuf_sz, p  ,off,dr,dg,db));
        rgb[GREEN] = clamp(interp(lutrgb, lutbuf_sz, p+1,off,dr,dg,db));
        rgb[BLUE]  = clamp(interp(lutrgb, lutbuf_sz, p+2,off,dr,dg,db));
