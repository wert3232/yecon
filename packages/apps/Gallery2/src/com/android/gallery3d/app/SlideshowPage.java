...
public class SlideshowPage extends ActivityState {
    ...
    private void showPendingBitmap() {
        ...
        Log.i(TAG, "system total memory is " + (outInfo.totalMem >>10)+"k");
        Log.i(TAG, "system available memory is " + (outInfo.availMem >>10)+"k");
        Slide slide = mPendingSlide;
        if (slide == null) {
			Log.i(TAG, "mPendingSlide is Null-----------");
            if (mIsActive) {
				Log.i(TAG, "mIsActive is true-----------");
		...
		Log.i(TAG, "showPendingBitmap---------" + slide.item.getPath().toString());
