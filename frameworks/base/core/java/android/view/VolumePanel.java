...
import android.widget.TextView;								//Jade add;
...
public class VolumePanel extends Handler implements OnSeekBarChangeListener, View.OnClickListener
{
    ...
    private static final int[] UNIFIED_VOLUME_STREAM = {
        AudioManager.STREAM_MUSIC, AudioManager.STREAM_GIS, AudioManager.STREAM_RING, 
        AudioManager.STREAM_BLUETOOTH_SCO, AudioManager.STREAM_RDS
    };
    ...
    private TextView mVolumeText;						//Jade add to show volume text

    private enum StreamResources {
        /**
         * jy 20141124 add for GIS and BACKCAR
         */
        BackcarStream(AudioManager.STREAM_BACKCAR,
                R.string.volume_icon_description_media,
                R.drawable.ic_audio_vol,
                R.drawable.ic_audio_vol_mute,
                true),
        GisStream(AudioManager.STREAM_GIS,
                R.string.volume_icon_description_media,
                R.drawable.ic_audio_vol,
                R.drawable.ic_audio_vol_mute,
                true),
    ...
    private static final StreamResources[] STREAMS = {
         ...
         * jy 20141124 add for GIS and BACKCAR
         */
        StreamResources.BackcarStream,
        StreamResources.GisStream,
    ...
    public VolumePanel(final Context context, AudioService volumeService) {
        ...
        mVolumeText = (TextView)mView.findViewById(R.id.volume_text);
    ...
    private void createSliders() {
        if (LOGD) {
            Log.e(TAG, "VolumePanel - createSliders - start");
        }
        ...
        if (LOGD) {
            Log.e(TAG, "VolumePanel - createSliders - end");
        }
    }
    ...
    private void updateVolumeText(StreamControl sc) {
		int hasVolume = SystemProperties.getInt("persist.sys.hasVolume", 0);
        int iVolume = getStreamVolume(sc.streamType);	  
        if (LOGD) {
            Log.e(TAG, "updateVolumeText - iVolume: " + iVolume + "  : " + hasVolume);
        }
        //if(sc.streamType == AudioSystem.STREAM_BLUETOOTH_SCO);
        //	iVolume --;
        String strVolume = Integer.toString(iVolume); 					//Jade add;
        if (iVolume < 10) {
            strVolume = strVolume;
        }
		mVolumeText.removeCallbacks(checkVolumeSynRun);
		if(iVolume == 0 && hasVolume != 0){
			syncount = 10 ;
			//mVolumeText.setText(strVolume + "(" + hasVolume + ")");	
			mVolumeText.setText(String.format("%01d",hasVolume));
			mVolumeText.postDelayed(checkVolumeSynRun, 100);
			return;
		}
        mVolumeText.setText(strVolume);		
    }
	private void updateVolumeText(StreamControl sc,int flag) {
		int hasVolume = SystemProperties.getInt("persist.sys.hasVolume", 0);
        int iVolume = getStreamVolume(sc.streamType);	  
        if (LOGD) {
            Log.e(TAG, "updateVolumeText - iVolume: " + iVolume + "  : " + hasVolume);
        }
        String strVolume = Integer.toString(iVolume); 					
        if (iVolume < 10) {
            strVolume = strVolume;
        }
        mVolumeText.setText(strVolume);		
	...
	private Runnable checkVolumeSynRun = new Runnable() {
		@Override
		public void run() {
			int hasVolume = SystemProperties.getInt("persist.sys.hasVolume", 0);
			if (LOGD) {
				Log.e(TAG, "updateVolumeText - hasVolume: "  + hasVolume + " syncount:" + syncount);
			}
			try{
				if(hasVolume == 0){
					mVolumeText.setText(Integer.toString(0));	
					syncount = 0;
				}else{
					if(syncount > 0){
						syncount -- ;
				...
				Log.e(TAG, "updateVolumeText - e: "  + e.toString());
			}
		}
	};	
    ...
    private void updateSlider(StreamControl sc) {
        if (LOGD) {
            Log.d(TAG, "updateSlider - start - sc.streamType: " + sc.streamType);
        }
        				
        sc.seekbarView.setProgress(getStreamVolume(sc.streamType));

        // updateVolumeText(sc);						//Jade add;
        final boolean muted = isMuted(sc.streamType);
        if (LOGD) {
            Log.d(TAG, "updateSlider - muted: " + muted);
        }
    ...
    public void postVolumeChanged(int streamType, int flags) {
        if (LOGD) {
            Log.e(TAG, "VolumePanel - postVolumeChanged - start - flags: " + flags);
        }
        if (mUnifiedVolumeAdjust && (flags & AudioManager.FLAG_SHOW_UI) == 0) {
            return;
        }
        ...
        if (LOGD) {
            Log.e(TAG, "VolumePanel - postVolumeChanged - end");
        }
    }
    ...
    public void postMuteChanged(int streamType, int flags) {
        if (LOGD) {
            Log.e(TAG, "VolumePanel - postMuteChanged");
        }
        if (hasMessages(MSG_VOLUME_CHANGED)) return;
        synchronized (this) {
            if (mStreamControls == null) {
...
//                setRingerIcon();
        ...
        if (sc != null) {
            ...
            if (LOGD) {
                Log.e(TAG, "onShowVolumeChanged - index: " + index);
            }
            if (index > 0) {
                sc.icon.setImageResource(sc.iconRes);
            } else if (index == 0) {
                sc.icon.setImageResource(sc.iconMuteRes);
            }
            updateVolumeText(sc);					//Jade add;
        ...
        if (!mDialog.isShowing()) {
            ...
            //Intent intent = new Intent(VOLUME_PANEL_DIALOG_SHOW);
    ...
    private void setMusicIcon(int resId, int resMuteId) {
        ...
        if (sc != null) {
            if (LOGD) {
                Log.e(TAG, "setMusicIcon - sc.streamType: " + sc.streamType + isMuted(sc.streamType));
            }
            sc.iconRes = resId;
    ...
    public void handleMessage(Message msg) {
        if (LOGD) {
            Log.e(TAG, "VolumePanel - handleMessage - msg.what: " + msg.what);
        }
        switch (msg.what) {
            ...
            case MSG_TIMEOUT: {
                if (mDialog.isShowing()) {
                    //Intent intent = new Intent(VOLUME_PANEL_DIALOG_DISMISS);
                    //mContext.sendBroadcast(intent);
    ...
    private void forceTimeout() {
        removeMessages(MSG_TIMEOUT);
        sendMessageDelayed(obtainMessage(MSG_TIMEOUT), 300);
    }
    ...
    public void onProgressChanged(SeekBar seekBar, int progress,
        ...
        if (fromUser && tag instanceof StreamControl) {
            ...
            if (getStreamVolume(sc.streamType) != progress) {
                if (LOGD) {
                    Log.e(TAG, "streamType: " + sc.streamType + " - progress: " + progress);
                }
				...
				if(sc.streamType == 3){
					if( progress == 0){
						...
						mProgress = progress;
						mContext.sendBroadcast(new Intent("action.system.volume.is_zero"));
					}else{
						sc.icon.setImageResource(sc.iconRes);
					}
				}
				
                SystemProperties.set(PROPERTY_KEY_STREAM_MUSIC_VOLUME, progress + "");

                if (mUnifiedVolumeAdjust) {
                    for (int i = 0; i < UNIFIED_VOLUME_STREAM.length; i++) {
                        setStreamVolume(UNIFIED_VOLUME_STREAM[i], progress, 0);
                    }
                    ...
                    int mark = YeconConstants.SRC_VOLUME_UNMUTE;
                    if (muted) {
                        mark = YeconConstants.SRC_VOLUME_MUTE;
                    }
                    mAudioManager.setYeconVolumeMute(AudioManager.STREAM_MUSIC, muted, 0, mark);
                } else {
                    setStreamVolume(sc.streamType, progress, 0);
                }
                Log.e(TAG, "onProgressChanged!!!");
                updateVolumeText(sc,0);
            }
        }
        resetTimeout();
    }
