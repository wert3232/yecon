...
public class Build {
    ...
    public static final String SERIAL = readEmmcId();//getString("ro.serialno");
    ...
    private static String getSerial() {
        //return SystemProperties.get("ro.serialno", SystemProperties.get("persist.sys.serialno", " "));
    	return readEmmcId();
    }
	...
	public static String readEmmcId() {
		...
		try {
			...
			if (localOb != null) {
				...
				if (localOb != null) {
					localOb = new FileReader("/sys/block/mmcblk0/device/cid"); // nand
																				// ID
					String number = (new BufferedReader((Reader) localOb).readLine());
					number = number.substring(number.length() - 16, number.length());
					myMacAddress = number.toUpperCase();
				}
			}
		} catch (Exception e1) {
			e1.printStackTrace();
		...
		Log.e(TAG, "readEmmcId-0--->" + myMacAddress);
		return myMacAddress;
	}
}
